...
本节先把线程、索引和边界检查打稳，下一节再把 Shared Memory 引入 GEMM 优化，形成从基础到优化的自然过渡。
...
torch::Tensor vector_add_cuda(torch::Tensor x, torch::Tensor y) {
    ...
    // 确保 CUDA 执行完成 (同步)
    // 这里保留一次显式同步，便于 Notebook 立刻验证结果；实际工程更常用 stream 同步或让上层框架接管。
...
### 性能对比：PyTorch vs CUDA JIT

这段基准只负责建立最基本的性能感知：先看 PyTorch 原生 `x + y`，再看本节的 CUDA JIT 扩展。Triton 的同类对比会在 Part 3 的算子章节里展开。


```python
import triton

def compare_pytorch_cuda_jit():
    if not torch.cuda.is_available():
        print("⏭️ 无 GPU，跳过性能对比")
        return

    if 'vector_add_extension' not in globals():
        print("⏭️ CUDA 扩展未编译，跳过性能对比")
        return
    ...
    ms_pt = triton.testing.do_bench(lambda: x + y)
    ms_cu = triton.testing.do_bench(lambda: vector_add_extension.vector_add_cuda(x, y))

    print(f"PyTorch Time: {ms_pt:.4f} ms")
    print(f"CUDA JIT Time: {ms_cu:.4f} ms")
    print(f"Speedup: {ms_pt / ms_cu:.2f}x")

compare_pytorch_cuda_jit()
...
- **线程分支发散**:
  ...
  - 影响通常很小，因为只有最后一个 warp 受影响

- **与下一节的关系**:
  - 本节先把线程、索引和边界检查打稳
  - 16 节会正式引入 Shared Memory 和 tiling，进入 GEMM 优化
  - 这里不提前展开共享内存代码，避免把入门页写成优化页

- **性能对比建议**:
  - 本节先对比 PyTorch 原生 `x + y` 和本节的 CUDA JIT 扩展
  - Triton 的同类对比留给 Part 3 的算子章节
  - 对向量加法这类轻算子，重点看“方法”和“边界行为”

- **JIT编译与同步语义**:
  - load_inline 适合快速原型和测试
  - 本例保留一次显式同步，只是为了 Notebook 里立即验证结果
  - 生产环境里更常见的是 stream 同步，或让上层训练/推理框架接管

- **常见错误排查**:
  - 编译失败：检查CUDA Toolkit安装
  - 运行时错误：使用cudaGetLastError()检查
  - 性能问题：使用nvprof或Nsight分析
### 关于 `cudaDeviceSynchronize()`

- 在 Notebook 中保留它，是为了让结果立即可见、便于验收。
- 在生产环境里，显式的 device 级同步通常会阻塞并发，不是默认推荐做法。
- 真正工程化时，更常依赖 stream 同步或上层框架调度。

### 🔜 下一步：从入门到优化

15 节展示了如何在 CUDA 中实现一个正确的 Kernel。

但“正确”只是起点。要理解 Triton 的 `tl.load` 为什么快，我们需要回答一个问题：

**如果不用 Shared Memory，GEMM 的性能会差多少？**

下一节（16. CUDA Shared Memory Optimization）将回答这个问题：
- 为什么矩阵乘法必须用 Shared Memory？
- Triton 的 SRAM 管理在 CUDA 层面对应什么操作？
- 手写 Shared Memory GEMM 和 Triton `tl.dot` 的差距在哪里？
...
- Triton适合日常开发
- 理解底层有助于写出更好的高层代码
**3. 为什么这里不使用 Shared Memory？**

向量加法本身是一个很轻的算子，直接用全局内存和线程索引就足够了。这里的重点不是做优化，而是先把 CUDA 的索引、边界和 JIT 扩展打稳；16 节会再把 Shared Memory 引入 GEMM 优化。
