# 09. Triton PagedAttention | KV Cache 间接寻址
...
>
> [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/datawhalechina/llm-algo-leetcode/blob/main/03_Triton_Kernels/09_Triton_PagedAttention.ipynb)
...
本节我们将用 Triton 编写一个极简版的 PagedAttention 解码阶段 (Decoding) 前向内核。这也是 vLLM 推理引擎的一个重要基石。它和 08. FlashAttention 的关系很直接：08 处理连续内存的 KV Cache，主要面向 Prefill 阶段；09 处理碎片化内存的 KV Cache，主要面向 Decoding 阶段，核心区别在于读取 K/V 之前先通过 `block_tables` 做一次间接寻址。
