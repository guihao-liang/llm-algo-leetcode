...
这一节先把 Qwen 和 Gemma 分别改了什么、为什么这么改讲清楚。
...
- **这一题要解决什么：** 把模型里的两个小型结构改动落到可运行代码里。
- **为什么这样做：** Gemma 的 `+1` 缩放和 Qwen 的权重绑定，本质上都是围绕稳定性和参数布局的工程取舍。
- **带走的直觉：** 架构 trick 不是“花活”，而是对训练稳定性、内存和表达能力的具体权衡。
