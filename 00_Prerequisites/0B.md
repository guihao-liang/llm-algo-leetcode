...
- 先看 [05](./05_PyTorch_Tensor_Fundamentals.ipynb) -> [06](./06_PyTorch_Tensor_Layout_and_Indexing.ipynb) -> [07](./07_PyTorch_Autograd_and_Backward.ipynb) -> [08](./08_PyTorch_Grad_Hygiene_and_No_Grad.ipynb)，把 Tensor 表达、布局、梯度回传和梯度习惯先串起来。若目标是尽快进入 Part 2，优先把 [06](./06_PyTorch_Tensor_Layout_and_Indexing.ipynb) 和 [07](./07_PyTorch_Autograd_and_Backward.ipynb) 读透，它们最直接对应后面的维度变换、Embedding、反向传播和训练闭环。
